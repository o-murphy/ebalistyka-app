...
```
`ReticleFullscreen` і `HomeScreen`: `ballisticAdj + manualAdj` → `offsetMil`.  
...
| `ReticleFullscreenScreen` | балістична + барабани | з `ReticleSettings.targetSizeMm` + `conditions.targetDistance` |
| `DistanceConvertorScreen` | `(0, 0)` | з angular-input конвертора |

### ReticleFullscreen — entity та screen
...
- [x] Ентіті `ReticleSettings` (bounded до `Owner`, як решта налаштувань):
  ...
  - [x] `targetSizeMm` — фізичний розмір мішені (мм), default `200.0`
...
- [ ] Screen display
  ...
  - [ ] Bottom block — `ListView`:
    - [ ] `verticalAdjustment` з unit selector (MOA, MRAD, MIL, cm/100m, in/100yd, clicks)
    - [ ] `horizontalAdjustment` з unit selector
    - [ ] `targetImage` — image selector (reusable, аналог `reticleImage` в SightWizardScreen)
    - [ ] `targetSize` з unit selector (mm / inch)
    - [ ] `reticleImage` — контролює `profile.sight.reticleImage`
    - [ ] `horizontalClick` / `verticalClick` — контролюють `profile.sight`
...
2. `DistanceConvertorScreen` + Phases 4–6 (ReticleFullscreen VM + Screen + Router) — паралельно після Phase 3
