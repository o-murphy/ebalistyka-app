...
- [ ] Додати `assets/svg/targets/` до `pubspec.yaml`
- [ ] Додати `make gen-targets` до Makefile
- [ ] `targetSvgProvider(id)` + `targetListProvider` — провайдери за аналогією з `reticleSvgProvider`
...
- [ ] `ReticleView` витягнути з `home_reticle_page.dart` у `lib/shared/widgets/reticle_view.dart`
- [ ] Ентіті `ReticleSettings` (bounded до `Owner`, як решта налаштувань):
  - [ ] `verticalAdjustmentRad` — додаткова поправка (барабани), додається до балістичної
  - [ ] `horizontalAdjustmentRad`
  - [ ] `targetImage` — tag SVG-мішені
  - [ ] `targetSizeMm` — фізичний розмір мішені (мм), default `200.0`
- [ ] Screen display
